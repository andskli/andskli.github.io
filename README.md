...
| `make mongodb-unpacked` | Install locked npm dependencies, test, and build only MongoDB Unpacked |
...
`make apps` is the shared entry point for all app builds. Both `make build` and `make serve`, as well as both GitHub Actions jobs, call it before Zola. Individual targets such as `make mongodb-unpacked` remain available for working on one app.
...
`make mongodb-unpacked` installs dependencies with `npm ci`, checks formatting, runs the model tests, and generates `static/mongodb-unpacked/`. It is included in `make apps`. If you invoke Zola directly, run `make apps` first.
